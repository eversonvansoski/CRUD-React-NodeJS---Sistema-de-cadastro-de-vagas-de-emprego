...
const usuario = require("../database/UsuariosDB");
    ...
    if (!nome || !email || !senha || !confirmarSenha) {
      ...
      bcrypt.hash(senha, saltRounds, async function (err, hash) {
        if (err) {
          ...
          let cadastroExistente = await usuario.verificaUsuarioExistente(email);
          if (!cadastroExistente) {
            let criarCadastro = await usuario.criarUsuario(
              nome,
              email,
              senhaHash,
